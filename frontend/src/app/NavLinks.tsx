      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute top-full right-0 mt-4 w-48 bg-background border border-border rounded-lg shadow-sm flex flex-col py-2 z-50">
            ...
            {isAdmin && (
              <>
                <Link href="/admin" onClick={() => setOpen(false)} className="px-4 py-3 text-sm hover:bg-accent/50 transition-colors">Admin</Link>
                <Link href="/eval" onClick={() => setOpen(false)} className="px-4 py-3 text-sm hover:bg-accent/50 transition-colors">Eval</Link>
              </>
            )}
          </div>
        )}
