    ...
    <div className="flex-1 bg-background text-foreground min-h-screen">
      <div className="container mx-auto p-4 md:p-8 max-w-5xl space-y-12 pt-12 pb-32">
        <div className="border-b border-border pb-6">
          ...
          <p className="text-muted-foreground font-medium uppercase tracking-widest text-sm mt-3">Test your UFC knowledge against our Generative AI.</p>
        </div>

        <div className="grid md:grid-cols-2 gap-8">
          <Card className="bg-card border-primary/20 shadow-2xl rounded-none">
            ...
            <CardContent className="pt-8">
              <form id="quiz-form" onSubmit={handleGenerate} className="space-y-8">
                <div className="space-y-3">
                  <Label htmlFor="topic" className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Topic (e.g. Khabib, UFC 300, Rules)</Label>
                  <Input
                    id="topic"
                    value={topic}
                ...
                <div className="space-y-3">
                  <Label htmlFor="difficulty" className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Difficulty</Label>
            ...
            <CardFooter className="bg-muted/30 pt-6 border-t border-border">
              <Button type="submit" form="quiz-form" disabled={loading || !topic} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground h-14 font-bold uppercase tracking-wider rounded-none text-lg">
                {loading ? (
                  <>
          ...
          <Card className="bg-card border-border shadow-2xl rounded-none flex flex-col">
            <CardHeader className="border-b border-border bg-muted/30">
              <CardTitle className="text-2xl font-black uppercase tracking-tighter text-foreground">Recent Quizzes</CardTitle>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border hover:bg-transparent">
                      <TableHead className="text-muted-foreground font-bold uppercase tracking-widest text-xs h-12">Topic</TableHead>
                      <TableHead className="text-muted-foreground font-bold uppercase tracking-widest text-xs h-12">Difficulty</TableHead>
                      <TableHead className="text-right text-muted-foreground font-bold uppercase tracking-widest text-xs h-12 pr-6">Action</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {sessions.length === 0 ? (
                      ...
                      sessions.map((session: QuizSession) => (
                        <TableRow key={session.id} className="border-border hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => router.push(`/quiz/${session.id}`)}>
                          ...
                          <TableCell className="text-right pr-6">
                            <Button variant="ghost" size="sm" className="text-primary hover:text-primary-foreground hover:bg-primary font-bold uppercase tracking-wider rounded-none px-4">
                              View
                            </Button>
                          </TableCell>
                        </TableRow>
