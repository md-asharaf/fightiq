      ...
      <div className="container mx-auto p-4 md:p-8 max-w-4xl space-y-12 pb-32 pt-12">
        <Card className="bg-card border-primary/30 text-center py-12 rounded-none shadow-sm relative overflow-hidden">
          ...
          <CardHeader className="relative z-10">
            ...
            <CardDescription className="text-lg mt-2 font-bold text-muted-foreground">Topic: {session.topic}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6 relative z-10">
            <div className={`text-7xl md:text-8xl font-bold ${percentage >= 70 ? 'text-green-500' : 'text-primary'}`}>
              {percentage}%
            </div>
          ...
          <CardFooter className="justify-center pt-8 relative z-10">
            <Button onClick={() => router.push("/quiz")} className="bg-foreground text-background hover:bg-muted-foreground font-bold rounded-none px-12 h-14">
              Back to Quizzes
            </Button>
          </CardFooter>
        </Card>
        ...
        <div className="space-y-8">
          <div>
            <h3 className="text-3xl font-bold tracking-tighter pb-4">Detailed Review</h3>
                ...
                <CardHeader className={`py-6 border-b ${isCorrect ? 'bg-green-500/10 border-green-500/20' : 'bg-primary/10 border-primary/20'}`}>
                  <div className="flex items-start justify-between gap-4">
                    <CardTitle className="text-xl leading-relaxed font-medium">
                      <span className="font-bold text-muted-foreground mr-2">Q{idx + 1}.</span> {q?.text}
                    </CardTitle>
