...
class QuizSessionRead(BaseModel):
    ...
    def strip_correct_answers(cls, data: Any) -> Any:
        if isinstance(data, dict):
            # If it's a dict, safely remove correct_option_id and explanation from questions
            ...
            for q in data.questions:
                ...
                if isinstance(safe_q, dict):
                    ...
                    safe_q.pop("explanation", None)
                sanitized_questions.append(safe_q)
            # Cannot mutate ORM model during read easily without breaking session,
            # so we create a dict representation.
            return {
                "id": data.id,
                "topic": data.topic,
...
class QuizSubmitRequest(BaseModel):
    ...
    # Dictionary mapping question_id -> selected_option_id
