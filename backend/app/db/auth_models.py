...
class User(Base):
    ...
    banReason: Mapped[str | None] = mapped_column(Text, nullable=True)
    banExpires: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
...
class Session(Base):
    ...
    id: Mapped[str] = mapped_column(String(255), primary_key=True)
    userId: Mapped[str] = mapped_column(String(255), ForeignKey("user.id", ondelete="CASCADE"), nullable=False)
...
class Verification(Base):
    ...
    expiresAt: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    createdAt: Mapped[datetime] = mapped_column(DateTime, nullable=True)
